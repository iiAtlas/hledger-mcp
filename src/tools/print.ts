...
const PrintInputSchema = CommonOptionsSchema.extend({
  query: z
    .string()
    .optional()
    .describe("Filter transactions using hledger query syntax"),
  ...
  format: z
    .string()
    .optional()
    .describe("Custom line format for each transaction"),
  explicit: z.boolean().optional().describe("Show only explicit amounts"),
  match: z
    .string()
    .optional()
    .describe("Highlight matches for the supplied regex"),
...
export class PrintTool extends QueryableTool<typeof PrintInputSchema> {
  readonly metadata: ToolMetadata<typeof PrintInputSchema> = {
    name: "hledger_print",
    description:
      "Print full transactions from the journal with optional filtering and formatting",
