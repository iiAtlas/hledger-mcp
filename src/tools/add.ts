...
const AddTransactionInputSchema = z.object({
  ...
  status: z
    .enum(["*", "!"])
    .optional()
    .describe("Transaction status: cleared (*) or pending (!)"),
  code: z.string().optional().describe("Optional transaction code"),
  description: z.string().min(1, "Description is required"),
  comment: z
    .string()
    .optional()
    .describe("Optional comment for the transaction header"),
  postings: z.array(PostingSchema).min(2, "At least two postings are required"),
  notes: z
    .array(z.string())
    .optional()
    .describe("Additional comment lines to append after postings"),
  dryRun: z
    .boolean()
    .optional()
    .describe("If true, validates without modifying the journal"),
});
...
> {
  readonly metadata: ToolMetadata<typeof AddTransactionInputSchema> = {
    ...
    description:
      "Add a new transaction to the journal file using structured input",
  ...
  protected async run(input: z.infer<typeof AddTransactionInputSchema>) {
    if (this.readOnly) {
      throw new ValidationError(
        "Add transactions are disabled while the server is running in read-only mode",
      );
    }
    ...
    return {
      ...
      command: result.applied
        ? `append-transaction ${result.journalPath}`
        : `dry-run-append ${result.journalPath}`,
      duration,
    };
  }
  ...
  private buildTransaction(
    input: z.infer<typeof AddTransactionInputSchema>,
  ): string {
    ...
    input.postings.forEach((posting) => {
      ...
      if (posting.tags) {
        const tagComments = Object.entries(posting.tags).map(
          ([key, value]) => `${key}: ${value}`,
        );
