...
const BalanceInputSchema = CommonOptionsSchema.extend({
  ...
  cumulative: z.boolean().optional().describe("Show cumulative changes from report start"),
  ...
  average: z.boolean().optional().describe("Show average column in multi-period reports"),
  rowTotal: z.boolean().optional().describe("Show row total column"),
  noTotal: z.boolean().optional().describe("Omit final total row"),
  sortAmount: z.boolean().optional().describe("Sort by amount instead of account name"),
  ...
  // Layout options
  layout: z.enum(["wide", "tall", "bare", "tidy"]).optional().describe("How to layout multi-commodity amounts"),
  noElide: z.boolean().optional().describe("Don't compress boring parent accounts"),
  format: z.string().optional().describe("Custom line format for single-period reports")
});
...
export class BalanceTool extends QueryableTool<typeof BalanceInputSchema> {
  readonly metadata: ToolMetadata<typeof BalanceInputSchema> = {
    ...
    description: "Show account balances, balance changes, or other balance-related reports with extensive customization options",
